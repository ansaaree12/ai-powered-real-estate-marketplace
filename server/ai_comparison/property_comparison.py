...
# ✅ Prepare Features for Model (Fixed "sqft" issue)
def prepare_features(properties):
    ...
    for prop in properties:
        ...
        # ✅ Convert "7000 sqft" → 7000.0 before passing to model
        ...
        feature_list.append([
            ...
            1 if prop.get("city", "").strip().lower() == "south mumbai" else 0,
            *amenities_encoded
        ])
...
# ✅ Generate AI Insights (Fixed "sqft sqft" issue & balanced 4 insights per property)
def generate_ai_insights(property, best_property, is_recommended=False):
    try:
        ...
        if is_recommended:
            ...
            reasons.append(f"📍 Prime Location: {city} (High demand area)")
            ...
            if city != best_city:
                reasons.append(f"📍 Location Tradeoff: {city} (Best property is in {best_city})")
            else:
        ...
        return "\n".join(reasons[:4])  # ✅ Ensures only 4 insights per property

    except Exception:
        return "AI insights unavailable."

# ✅ API Endpoint (Now returns structured JSON)
...
def compare_properties():
    try:
        ...
        for prop in properties:
            ...
            prop["view_url"] = f"/properties/{prop['_id']}"  # ✅ Fixed frontend URL
